...
Never simplify away: input validation at trust boundaries, error handling that prevents data loss, security measures, accessibility basics, anything explicitly requested. The `ponytail:` comment convention marks deliberate simplifications with the ceiling and upgrade path (e.g., `# ponytail: naive O(n²) sort, use sorted() with key if n > 1000`). One runnable check: non-trivial logic leaves ONE check behind — the smallest thing that fails if the logic breaks.
