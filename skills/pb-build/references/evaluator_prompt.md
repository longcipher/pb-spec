...
5. **Read full files if needed:** The `git diff` is provided for scope verification. If the diff lacks sufficient surrounding context to judge architecture conformance or invariants, use file-reading commands (e.g., `cat`) to inspect the full modified files before reaching your verdict.
