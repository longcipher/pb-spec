...
**Continuous execution:** Do not pause to check in between tasks. Execute all tasks from the plan without stopping. The only reasons to stop are: BLOCKED status you cannot resolve, ambiguity that genuinely prevents progress, or all tasks complete. "Should I continue?" prompts and progress summaries waste time — you were asked to execute the plan, so execute it.
...
digraph when_to_use {
    ...
    "pb-subagent-driven-development" [shape=box];
    "pb-build" [shape=box];
    ...
    "Stay in this session?" -> "pb-subagent-driven-development" [label="yes"];
    "Stay in this session?" -> "pb-build" [label="no - full Generator/Evaluator"];
}
```

**vs. pb-build (full Generator/Evaluator):**
...
digraph process {
    ...
    "Use pb-finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Dispatch final code reviewer subagent" -> "Use pb-finishing-a-development-branch";
}
```
...
to the implementer and re-review.
...
- At skill start, check for a ledger:
  `cat "$(git rev-parse --show-toplevel)/.pb-spec/sdd/progress.md"`. Tasks listed there
...
- Final whole-branch review: use pb-requesting-code-review's code-reviewer template

## Example Workflow

```
You: I'm using pb-subagent-driven-development to execute this plan.
...
You: "User level (~/.config/pb-spec/hooks/)"
...
- Lighter weight for straightforward plans
